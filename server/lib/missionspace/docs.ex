defmodule Missionspace.Docs do
  @moduledoc """
  The Docs context.
  """
  ...
  def list_starred_doc_folders(workspace_id, user, opts \\ []) do
    starred_ids = Missionspace.Stars.starred_ids(user.id, "doc_folder")
  ...
  defp filter_accessible_folders(query, %{id: user_id}) do
    project_folder_ids =
      Missionspace.Projects.get_project_item_ids_for_user_projects(user_id, "doc_folder")

    item_member_ids = Missionspace.Projects.get_user_item_member_ids(user_id, "doc_folder")
  ...
  def delete_doc_folder(%DocFolder{} = folder) do
    ...
    case result do
      {:ok, _} -> Missionspace.Namespaces.release_prefix("doc_folder", folder.id)
      _ -> nil
    end
  ...
  def list_starred_docs(workspace_id, user_id, opts \\ []) do
    starred_ids = Missionspace.Stars.starred_ids(user_id, "doc")
