defmodule Missionspace.Lists do
  @moduledoc """
  The Lists context.
  """
  ...
  alias Missionspace.Lists.{List, ListStatus, Task}
  ...
  def list_starred_lists(workspace_id, user, opts \\ []) do
    starred_ids = Missionspace.Stars.starred_ids(user.id, "board")
  ...
  defp filter_accessible_lists(query, %{id: user_id}) do
    ...
    project_list_ids =
      Missionspace.Projects.get_project_item_ids_for_user_projects(user_id, "list")

    item_member_ids = Missionspace.Projects.get_user_item_member_ids(user_id, "list")
  ...
  def create_list(attrs \\ %{}) do
    Repo.transaction(fn ->
      with {:ok, list} <-
             ...
             |> Repo.insert(),
           {:ok, _prefix} <-
             Missionspace.Namespaces.reserve_prefix(
               list.prefix,
               "list",
               list.id,
               list.workspace_id
  ...
  def delete_list(%List{} = list) do
    ...
    case result do
      {:ok, _} -> Missionspace.Namespaces.release_prefix("list", list.id)
      _ -> nil
    end
  ...
  defdelegate suggest_prefix(name, workspace_id), to: Missionspace.Namespaces
  defdelegate check_prefix_available?(prefix, workspace_id), to: Missionspace.Namespaces
  ...
  def list_starred_tasks(workspace_id, user_id) do
    starred_ids = Missionspace.Stars.starred_ids(user_id, "task")
