defmodule MissionspaceWeb.ItemMemberController do
  ...
  def create(conn, %{"user_id" => user_id}) do
    ...
    with {:ok, _user} <- Accounts.get_workspace_user(user_id, workspace_id),
      ...
      member = Missionspace.Repo.preload(member, :user)
  ...
  defp load_item("list", id, workspace_id), do: Missionspace.Lists.get_list(id, workspace_id)

  defp load_item("doc_folder", id, workspace_id),
    do: Missionspace.Docs.get_doc_folder(id, workspace_id)

  defp load_item("channel", id, workspace_id), do: Missionspace.Chat.get_channel(id, workspace_id)
