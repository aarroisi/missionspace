defmodule MissionspaceWeb.DocController do
  ...
  def index(conn, %{"starred" => "true"} = params) do
    ...
    entries = Missionspace.Stars.mark_starred(page.entries, user.id, "doc")
  ...
  def show(conn, _params) do
    user = conn.assigns.current_user
    doc = Missionspace.Stars.mark_starred(conn.assigns.doc, user.id, "doc")
        ...
        Task.start(fn ->
          ...
          Enum.each(added_mentions, fn user_id ->
            if user_id != current_user.id do
              case Missionspace.Notifications.create_notification(%{
                     type: "mention",
                     item_type: "doc",
                ...
                {:ok, notification} ->
                  notification = Missionspace.Repo.preload(notification, [:actor])
