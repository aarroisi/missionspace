defmodule MissionspaceWeb.ListController do
  ...
  def index(conn, params) do
    ...
    entries = Missionspace.Stars.mark_starred(page.entries, user.id, "board")
  ...
  def create(conn, params) do
    ...
    with {:ok, list} <- Lists.create_list(list_params) do
      ...
      list = Missionspace.Repo.preload(list, :created_by)
  ...
  def show(conn, _params) do
    user = conn.assigns.current_user
    list = Missionspace.Stars.mark_starred(conn.assigns.list, user.id, "board")
    tasks = Missionspace.Stars.mark_starred(list.tasks, user.id, "task")
