defmodule MissionspaceWeb.DirectMessageController do
  ...
  def index(conn, params) do
    ...
    entries = Missionspace.Stars.mark_starred(page.entries, user.id, "direct_message")
  ...
  def create(conn, %{"user2_id" => user2_id}) do
    ...
    with {:ok, direct_message} <-
      ...
      direct_message = Missionspace.Repo.preload(direct_message, [:user1, :user2])
  ...
  def show(conn, _params) do
    user = conn.assigns.current_user
    dm = Missionspace.Stars.mark_starred(conn.assigns.direct_message, user.id, "direct_message")
